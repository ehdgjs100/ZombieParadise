...
public class ShortZombieController : MonoBehaviour
{
    ...
    private float getHitTerm;
    public float updateInterval = 12.0f;
    private float timeSinceLastUpdate;

    public float cryTerm = 5.0f;
    ...
    [SerializeField] AudioClip[] zombieIdleClips; //#0,1Idle #2GetHit
    ...
    void Update()
    {
        CheckPicked();
        ZombieControl();
        if (!isChecked && !playerChecked) CheckRandomMove();

        if (cryTerm <= 0.0f) StartCoroutine(ZombieCry());
        ...
        getHitTerm -= Time.deltaTime;

    }
    public void GetShortWeaponHit(float dmg)
    {
        if(getHitTerm <= 0.0f)
        {
            audioSource.PlayOneShot(zombieIdleClips[2]);
            this.zombieHp -= dmg;
            getHitTerm = 1.5f;
        }
    }
    IEnumerator ZombieCry()
    {
        audioSource.PlayOneShot(zombieIdleClips[1]);

        float randomCryTerm = Random.Range(5.0f, 10.0f);
        yield return null;
    }
    void CheckRandomMove()
    {
        ...
        {
            nav.ResetPath();
            _zombieAnimator.SetTrigger("idle");
        }
