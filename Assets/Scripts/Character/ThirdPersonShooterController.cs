...
public class ThirdPersonShooterController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (starterAssetsInputs.aim && (ActionCtrl.isEquipedPistol || ActionCtrl.isEquipedWeapon))
        {
            //Animation Rigging on/off
    ...
    void UseShortWeapon()
    {
        ...
        else if (ActionCtrl.isEquipedShortWeapon)
        {
            ...
            {
                _animator.SetTrigger("AxeAttack");
                StartCoroutine(Swing(0));
            }
            else
            {
                _animator.SetTrigger("hitCrawl");
                StartCoroutine(Swing(1));
            }

        }
    }
    IEnumerator Swing(int type)
    {
        if (type == 0)
        {
            yield return new WaitForSeconds(0.3f);
            shortWeaponArea.enabled = true;
            shortWeaponArea.GetComponent<Melee>().damage = currentGun.damage * 2;
    ...
    void CreateBullet(Vector3 _mousePos, int weaponType)
    {
        if (weaponType == 1)
        {
            Instantiate(bulletPrefab, spawnBulletPosRifle.position, spawnBulletPosRifle.rotation);
        }
        else
        {
            Instantiate(bulletPrefab, spawnBulletPosRifle.position, spawnBulletPosPistol.rotation);
        }
